...
const TicketTimeTracker: React.FC<TicketTimeTrackerProps> = ({ tickets }) => {
      ...
      <div className="bg-white rounded-lg w-max ">
        ...
        <div className="grid sm:grid-cols-[300px,250px] grid-cols-[200px,150px] gap-x-4 font-normal pb-2 mb-2">
          ...
          <div className='2xl:text-md text-[10px] sm:text-sm italic text-[#000000b0]'  >Time Logged</div>
           ...
           <span className='absolute  xl:left-[390px] xl:top-[257px] lg:left-[390px] lg:top-[255px] 2xl:left-[825px] 2xl:top-[360px] md:top-[572px] md:left-[56%] sm:left-[57%] left-[54%]'>
  ...
  <div className='2xl:mt-8'>
        {/* Ticket Rows */}
        {tickets.map((ticket) => (
          <div key={ticket.id} className="grid sm:grid-cols-[300px,250px] grid-cols-[230px,130px]  sm:gap-x-4 gap-x-1 items-center gradient-border-b  xl:py-4 sm:py-3 py-1  ">
            ...
            <div className="flex items-center sm:pr-6   justify-normal">
              ...
              <span className="font-medium  md:text-[13px] sm:text-[12px] text-[9px] whitespace-nowrap ">{ticket.id}</span>
              <span className="ml-2 text-black font-normal  md:text-[13px] sm:text-[12px] text-[9px]  whitespace-nowrap">{ticket.title}</span>
            </div>

            {/* Right Column - Time Logged */}
            <div >
              <span className='font-normal 2xl:text-3xl md:text-[13px] sm:text-[12px] text-[9px] sm:pl-8 '>{ticket.timeLogged}</span>
...
// 2xl:text-3xl lg:text-[9px] xl:text-[12px] 


//2xl:text-3xl lg:text-[10px] xl:text-sm 
